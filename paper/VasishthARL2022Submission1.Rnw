...
in the \citet{grodner} data,  the intercept and slope on the log ms scale are approximately $6$ and $0.12$ log ms respectively, and the standard error of the slope is $0.05$ log ms. If we take these estimates as an initial guess at the range of plausible effect sizes, the effect can be assumed a priori to approximately range from \Sexpr{round(exp(6 + (0.12-0.10)/2) - exp(6 - (0.12-0.10)/2 ))} ms to \Sexpr{round(exp(6 + (0.12+0.10)/2) - exp(6 - (0.12+0.10)/2 ))} ms.
\begin{marginnote}[]
...
\item An agnostic or uninformative prior, Normal(0,1), that allows a wide range of possible values ranging from \Sexpr{round(exp(6 + (-2/2)) - exp(6 - (-2)/2 ))} ms to \Sexpr{round(exp(6 + (2)/2) - exp(6 - (2)/2 ))} ms; here, too, the null hypothesis is that $\beta=0$;
...
\hline
English  & $\beta=0$  & $\beta \sim Normal(0,0.1)$      &   \Sexpr{round(bfEN,2)}    &    \Sexpr{round(mean(RC_EN$RC_EN))} [\Sexpr{round(quantile(RC_EN$RC_EN,prob=0.025))}, \Sexpr{round(quantile(RC_EN$RC_EN,prob=0.975))}]   \\
        & $\beta=0$      &  $\beta \sim  Normal(0,1)$     &     \Sexpr{round(bfEN2normal,2)}     &   \Sexpr{round(mean(RC_EN2normal))} [\Sexpr{round(quantile(RC_EN2normal,prob=0.025))}, \Sexpr{round(quantile(RC_EN2normal,prob=0.975))}]  \\
%        & $\beta=0$      &  $\beta \sim  Cauchy(0,1)$     &     \Sexpr{round(bfEN2,2)}     &   \Sexpr{round(mean(RC_EN2))} [\Sexpr{round(quantile(RC_EN2,prob=0.025))}, \Sexpr{round(quantile(RC_EN2,prob=0.975))}]  \\
        & $\beta = 0$      &  $\beta \sim  Normal(0.02,0.01)$     &     \Sexpr{round(bfEN3zero,2)}     &   \Sexpr{round(mean(RC_EN3zero))} [\Sexpr{round(quantile(RC_EN3zero,prob=0.025))}, \Sexpr{round(quantile(RC_EN3zero,prob=0.975))}]  \\
        & $\beta \sim Normal(0,0.01)$      &  $\beta \sim  Normal(0.02,0.01)$     &     \Sexpr{round(bfEN3,2)}     &   \Sexpr{round(mean(RC_EN3))} [\Sexpr{round(quantile(RC_EN3,prob=0.025))}, \Sexpr{round(quantile(RC_EN3,prob=0.975))}]  \\
\hline
Chinese  & $\beta=0$ &   $\beta \sim Normal(0,0.1)$      &   \Sexpr{round(bfCN,2)}    &    \Sexpr{round(mean(RC_CN$RC_CN))} [\Sexpr{round(quantile(RC_CN$RC_CN,prob=0.025))}, \Sexpr{round(quantile(RC_CN$RC_CN,prob=0.975))}]   \\
         & $\beta=0$   &   $\beta \sim  Normal(0,1)$     &     \Sexpr{round(bfCN2normal,2)}     &   \Sexpr{round(mean(RC_CN2normal))} [\Sexpr{round(quantile(RC_CN2normal,prob=0.025))}, \Sexpr{round(quantile(RC_CN2normal,prob=0.975))}]  \\
%         & $\beta=0$   &   $\beta \sim  Cauchy(0,1)$     &     \Sexpr{round(bfCN2,2)}     &   \Sexpr{round(mean(RC_CN2))} [\Sexpr{round(quantile(RC_CN2,prob=0.025))}, \Sexpr{round(quantile(RC_CN2,prob=0.975))}]  \\
       & $\beta = 0$      &  $\beta \sim  Normal(-0.02,0.01)$     &     \Sexpr{round(bfCN3zero,2)}     &   \Sexpr{round(mean(RC_CN3zero))} [\Sexpr{round(quantile(RC_CN3zero,prob=0.025))}, \Sexpr{round(quantile(RC_CN3zero,prob=0.975))}]  \\
       & $\beta \sim Normal(0,0.01)$      &  $\beta \sim  Normal(-0.02,0.01)$     &     \Sexpr{round(bfCN3,2)}     &   \Sexpr{round(mean(RC_CN3))} [\Sexpr{round(quantile(RC_CN3,prob=0.025))}, \Sexpr{round(quantile(RC_CN3,prob=0.975))}]  \\
\hline
\end{tabular}
\end{center}
\end{table}
